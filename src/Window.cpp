...
: QMainWindow(NULL)
{
    ...
    cout << "first window creation" << endl;

    best_results = LearningQueueWrapper(0);

    cout << "after best results creation" << endl;
}
...
void ShowWindow::loadScene(void)
{
    ...
    cout << endl << "Inertia Matrix: "   << endl   << objects[0] -> getInertiaMatrix();
...
{
    for(test_index;test_index < total_tests;test_index+=1){
        ...
        iCubOptParameters icub_param = iCubOptimizable::initDefaultParams();
                        ...
                        icub_param.active_variables.push_back(TRANS_Z);
                        //icub_param.active_variables.push_back(ROT_ROLL);
                        //icub_param.active_variables.push_back(ROT_PITCH);
                        //icub_param.active_variables.push_back(ROT_YAW);
            ...
            (movehand_mode != ESPH       )   )
        {
            ...
            // Start Optimization
            TGPOptimization* tgp_opt  = new TGPOptimization(tgp_param, opt_param, (*func), LOG_LEARNING | MONTE_CARLO_OPT, false, test_index);
 ...
 *  Procecure                                                                                     *
 *                                                                                                *
 *  Description: updateGraspBox - updates combobox from user interface 
 *  Class      : ShowWindow                                                                       *
 **************************************************************************************************/
