 ...
 *  Procecure                                                                                     *
 *                                                                                                *
 *  Description: collidingJoints                                                                  *
 *  Class      : iCubOptimizable                                                                  *
 **************************************************************************************************/
int iCubOptimizable::collidingJoints(){

    CollisionModelPtr         col_model1 = _object -> getCollisionModel();
    vector<CollisionModelPtr> col_model2 = _robot  -> getCollisionModel();
    if ( _col_checker -> checkCollision(col_model2, col_model1) )
    {
        int n_colliding_actors =0;
        vector< CollisionModelPtr >::iterator it1 = col_model2.begin();
        
        while (it1 != col_model2.end())
        {
            float dist = _col_checker->calculateDistance(*it1, col_model1);
            if (dist==0){
        ...
        cout << "number of collision: " << n_colliding_actors << endl;
        return n_colliding_actors;
    }else{
        return 0;
    }
}
 ...
 *  Procecure                                                                                     *
 *                                                                                                *
 *  Description: evaluateCollision                                                                *
 *  Class      : iCubOptimizable                                                                  *
 **************************************************************************************************/
double iCubOptimizable::evaluateCollision()
{
    int result=0;
    result=collidingJoints();
    cout << "metric: " << -result *0.1 << endl;
...
void iCubOptimizable::showBestGrasps(uint index, LearningQueueWrapper& best_grasps)
{
    ...
    if (index < best_queries.size())
    {
        ...
        vectord query = best_queries[index];

        if (query.size() != _original_dim)
        {
            query = vectord(_original_dim);
            query[0] = best_queries[index][0];
            query[1] = best_queries[index][1];
            query[2] = 277.001;
            query[3] = 336.682;
            query[4] = 337.414;
            query[5] = 615.839;
        ...
        evaluateGraspQuality(applyQueryToHand(query), isforceclosure);
    }
}
